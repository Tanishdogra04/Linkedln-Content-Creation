...
const ContentCalendar = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'Published':
        return <span class="px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-600 text-[10px] font-bold flex items-center gap-1 shadow-sm"><CheckCircle2 size={10} /> Published</span>;
      case 'Scheduled':
        return <span class="px-2 py-0.5 rounded-full bg-indigo-50 border border-indigo-200 text-indigo-600 text-[10px] font-bold flex items-center gap-1 shadow-sm"><Clock size={10} /> Scheduled</span>;
      default:
        return <span class="px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-500 text-[10px] font-bold flex items-center gap-1 shadow-sm"><FileText size={10} /> Draft</span>;
    }
  };
    ...
    <div class="flex flex-col gap-8">
      ...
      <GlassCard hover={false} className="border-emerald-500/20 shadow-sm bg-emerald-50/30">
        <div class="flex items-center justify-between gap-4">
          <div>
            <h3 class="font-display font-extrabold text-xl mb-1 text-slate-800">Content Calendar</h3>
          ...
          <form onSubmit={handleCreateOrUpdate} class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="flex flex-col gap-4">
              <div class="flex flex-col gap-1.5">
                <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Internal Reference Title</label>
                <input
                  type="text"
                  required
                  value={title}
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Scheduled Date & Time</label>
                <input
                  type="datetime-local"
                  required
                  value={scheduledDate}
            ...
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Post content</label>
              <textarea
                required
                rows={9}
                value={post}
      ...
      {loading ? (
        <div class="flex flex-col items-center justify-center py-20">
          ...
          <p class="text-slate-500 text-sm">Fetching scheduled assets...</p>
        </div>
      ) : events.length === 0 ? (
        <div class="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-slate-300 rounded-2xl bg-white/50">
          ...
          <h4 class="font-display font-bold text-slate-600 text-base">Calendar is Empty</h4>
            ...
            <GlassCard key={event._id} hover={true} className="flex flex-col justify-between gap-5 relative border-slate-200 bg-white">
              <div>
                <div class="flex items-center justify-between gap-3 mb-3">
                  ...
                  <span class="text-[11px] text-slate-500 font-semibold font-mono">
                    {new Date(event.scheduledDate).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                
                <h4 class="font-bold text-slate-800 text-base line-clamp-1 mb-2">
                  {event.title}
                </h4>
                
                <p class="text-slate-600 text-xs line-clamp-4 leading-relaxed whitespace-pre-line bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                  {event.post}
                </p>
              </div>
