  ...
  if (loading) {
    return (
      <div class="flex flex-col items-center justify-center py-40">
        ...
        <p class="text-slate-500 text-sm">Aggregating creation records...</p>
      </div>
    );
  }
    ...
    <div class="flex flex-col gap-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <GlassCard className="flex items-center gap-5 border-slate-200 bg-white">
          ...
          <div>
            <p class="text-slate-500 text-xs font-semibold uppercase tracking-wider">Total Generated</p>
            <h3 class="text-3xl font-extrabold font-display text-slate-800 mt-1">{data?.totalGenerated}</h3>
          </div>
        </GlassCard>
        ...
        <GlassCard className="flex items-center gap-5 border-slate-200 bg-white">
          ...
          <div>
            ...
            <h3 class="text-3xl font-extrabold font-display text-slate-800 mt-1">{data?.totalResearch}</h3>
          </div>
        </GlassCard>

        <GlassCard className="flex items-center justify-between gap-5 sm:col-span-2 lg:col-span-1 border-slate-200 bg-white">
          <div class="flex items-center gap-5">
            <div class="h-12 w-12 rounded-xl bg-green-50 border border-green-100 flex items-center justify-center text-green-600">
              <TrendingUp size={22} />
            </div>
            <div>
              <p class="text-slate-500 text-xs font-semibold uppercase tracking-wider">Active Campaigns</p>
              <h3 class="text-3xl font-extrabold font-display text-slate-800 mt-1">4</h3>
            </div>
          </div>
          <button 
            onClick={fetchAnalytics}
            class="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-500 hover:text-slate-700 active:scale-95 transition-all shadow-sm"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <GlassCard hover={false} className="lg:col-span-7 border-slate-200 bg-white">
          <h4 class="font-display font-extrabold text-lg text-slate-800 mb-6 flex items-center gap-2">
